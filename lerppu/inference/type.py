...
connection_smells = {
    ...
    ConnectionType.PCIE: re.compile(
        r"(2.5\"|HHHL)[\s,]+(PCI\s*e)|PCIe-kortti|SFF, PCI|PCI Express 3.0 x4",
        re.IGNORECASE,
    ),
    ConnectionType.SAS: re.compile(
        r"SAS\s*[1236,]|Serial Attached SCSI", re.IGNORECASE
    ),
    ConnectionType.SATA: re.compile(
        r"SATA\s*[36]Gb|SATA-(600|150|300)|2.5\" SATA|ATA SSD|SATA$", re.IGNORECASE
    ),
...
def _find_smells(strings: Iterable[str], smell_map: dict[T, re.Pattern]) -> set[T]:
    smells = set()
    for s in strings:
...
def get_connection_type_from_data(
    *, name: str, description: str
) -> ConnectionType | None:
    try:
        smells = _find_smells((name, description), connection_smells)
        ...
        log.warning(
            "Can't get connection type for %r/%r: %s",
            name,
            description,
            exc,
        )
